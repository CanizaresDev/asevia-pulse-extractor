...
def run_query_to_df(parquet_path: pathlib.Path) -> pd.DataFrame:
    con = duckdb.connect(database=":memory:")
    sql = build_sql(parquet_path, null_to_zero=False).strip().rstrip(";")
    df = con.execute(sql).df()

    # Make the timestamps/dates friendly for Sheets
    # (Sheets likes strings; also avoids timezone formatting surprises)
...
def gspread_client() -> gspread.Client:
    ...
    if json_env:
        ...
        raise SystemExit(
            "Set GOOGLE_SERVICE_ACCOUNT_JSON or GOOGLE_SERVICE_ACCOUNT_FILE for Google Sheets auth."
        )

    return gspread.authorize(creds)


def write_df_to_sheet(df: pd.DataFrame) -> None:
    spreadsheet_id = require("GSHEETS_SPREADSHEET_ID")
    sheet_name = require("GSHEETS_SHEET_NAME")

    gc = gspread_client()
    sh = gc.open_by_key(spreadsheet_id)
    ws = sh.worksheet(sheet_name)

    # Clear the whole sheet first
    ws.clear()
    ...
    print(f"Wrote {len(df)} rows x {len(df.columns)} cols to '{sheet_name}' in {spreadsheet_id}")


def main() -> None:
    parquet_path = get_parquet_path()
    df = run_query_to_df(parquet_path)
